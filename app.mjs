...
function chart(){const {x,y}=profile,r=result,W=1100,H=280,p={l:58,r:24,t:20,b:44},xmin=x[0],xmax=x.at(-1),ymin=Math.max(0,Math.floor(Math.min(...y)/20)*20-20),ymax=Math.ceil(Math.max(...y)/20)*20+20;
 ...
 $('chartNote').textContent=`最小点：始点側 ${x[r.left]} px / 終点側 ${x[r.right]} px　｜　平均点数：${r.L.n} / ${r.R.n} / 中央 ${r.C.n} 点　｜　${sourceMode==='csv'?'CSVの輝度値を使用':'写真から読み取り（'+($('gray').value==='mean'?'RGB単純平均':'RGB加重平均')+'）'}`;
}
function render(){for(const [id,val] of [['k',result.K],['edge',result.edge],['center',result.C.mean],['left',result.L.mean],['right',result.R.mean]])$(id).textContent=fmt(val);
...
function recalc(){error(null);try{if(sourceMode==='image')profile=sampleLine(pixels,img.width,img.height,a,b,$('gray').value);result=analyze(profile);render();}catch(e){result=null;$('save').disabled=true;$('profileSave').disabled=true;for(const id of ['k','edge','center','left','right'])$(id).textContent='—';$('status').textContent='線を確認';$('warnings').replaceChildren();$('chart').replaceChildren();$('chartNote').textContent='';drawPhoto();error(e);}}
...
$('apply').onclick=()=>{a={x:Number($('ax').value),y:Number($('ay').value)};b={x:Number($('bx').value),y:Number($('by').value)};if(['ax','ay','bx','by'].some(id=>$(id).value==='')){error('座標を入力してください。');return;}center={x:(a.x+b.x)/2,y:(a.y+b.y)/2};recalc();};
...
photo.onpointerdown=e=>{if(!img||sourceMode!=='image')return;const p=point(e);drag=Math.hypot(p.x-a.x,p.y-a.y)*view.s<18?'a':Math.hypot(p.x-b.x,p.y-b.y)*view.s<18?'b':'new';if(drag==='new'){a=p;b={...p};result=null;}photo.setPointerCapture(e.pointerId);drawPhoto();};
...
function resultRecord(){return {Source:name,Mode:sourceMode,Gray:sourceMode==='csv'?'Source CSV':$('gray').value,A_x:sourceMode==='image'?a.x:'',A_y:sourceMode==='image'?a.y:'',B_x:sourceMode==='image'?b.x:'',B_y:sourceMode==='image'?b.y:'',Left_min_x:profile.x[result.left],Left_min:profile.y[result.left],Right_min_x:profile.x[result.right],Right_min:profile.y[result.right],I_left:result.L.mean,I_right:result.R.mean,I_edge:result.edge,I_center:result.C.mean,N_left:result.L.n,N_right:result.R.n,N_center:result.C.n,K_percent:result.K??'',Status:result.warnings.join(' / ')||'OK'};}
